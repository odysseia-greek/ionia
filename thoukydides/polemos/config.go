...
func CreateNewConfig(ctx context.Context) (*Service, error) {
	...
	logging.System("Thoukydides configuration: connecting to tracing service")
	...
	logging.System("Thoukydides configuration: opening tracing stream")
	streamer, err := tracer.Chorus(ctx)
	if err != nil {
		return nil, fmt.Errorf("open tracing stream: %w", err)
	}

	logging.System("Thoukydides configuration: connecting to ambassador service")
	...
	logging.System("Thoukydides configuration: retrieving Elasticsearch credentials")
	...
	logging.System(fmt.Sprintf("Thoukydides configuration: creating Elasticsearch client for %s", elasticAddress))
	elastic, err := aristoteles.NewClient(elasticConfig)
	if err != nil {
		return nil, fmt.Errorf("create Elasticsearch client: %w", err)
	}
	logging.System("Thoukydides configuration: checking Elasticsearch health")
	...
	configured := NewService(version, &ElasticFormStore{Client: elastic, Index: index})
	configured.Elastic = elastic
	configured.Index = index
	configured.Streamer = streamer

	logging.System(fmt.Sprintf(`Thoukydides Configuration Overview:
